...
ros2_ws/
├── src/
│   └── my_robot_package/  <-- This is assumed to be the ROS package directory
│       ├── CMakeLists.txt
│       ├── package.xml
│       ├── launch/
│       │   └── robot_launch.py
│       ├── src/  <-- Python nodes are here
│       │    ├── ctd.py
│       │    ├── dvl.py
│       │    ├── float_simulator.py
│       │    └── gnss.py
...
*(Note: The `my_robot_package` directory should be placed inside `ros2_ws/src/` after cloning or moving from the `underwaterAutonomousRobot` repository if it's nested within.)*
...
* **To echo data from a specific topic (e.g., GNSS data):**
    ...
    ```
* **To echo the float's simulated depth:**
    ```bash
    ros2 topic echo /simulator/depth_m
    ```
* **To echo the defined seafloor Z position (should be constant):**
    ```bash
    ros2 topic echo /simulator/defined_seafloor_z_position_m
    ```
...
* To publish a target depth of, for example, 15.0 meters:
    ...
    * The topic name `/controller/target_depth_m` is based on the `float_simulator.py` subscriptions.
